...
每条流均物化 `s=0`、`s=40` 中间 checkpoint 和自动达到
`Rgen/Rgen0<=1e-6` 的最终点。下表用与主结果相同的最大
报告 `Nmax` 直接对角化 `s=40` Hamiltonian；“距最终”是两个
后 NCSM 本征值的绝对差，不是 MR 零体项差。

| 核 | `Nmax` | `s=0` | `s=40` | 最终 | `s=40` 距最终 |
|---|---:|---:|---:|---:|---:|
| He4 | 8 | -20.3388325043 | -20.2399215656 | -20.2399185504 | 0.0030 keV |
| Be8 | 4 | -25.1799853478 | -27.9014296322 | -27.9391351682 | 37.7055 keV |
| C12 | 4 | -40.2376145487 | -43.5292771211 | -43.5312362939 | 1.9592 keV |
| O16 | 4 | -62.2980557824 | -63.7200917317 | -63.7200911998 | 0.0005 keV |

四个 `s=40` 文件均由独立干净 checkout 中的同一 C++ bridge 读回，
证明中间点不仅存在于流轨迹 metadata，而是可被下游实际消费
的普通真空正规序 0B/1B/2B Hamiltonian。
