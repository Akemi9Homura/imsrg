...
3. 直接积分 `dH/ds=[eta,H]_(0,1,2B)`，记录每步残差和对称性误差。
   正式停止量 `Rgen` 与现有 `IMSRGSolver` 监测的 `Eta.Norm()` 是同一个
   物理量，即 Vobig 式 (6.5.28)--(6.5.29) 中完整的、带 EN 分母并
   反对称化后的掩码生成元范数；现有 C++ solver 使用绝对阈值，本计划
   明确采用相对初值的 `1e-6` 门槛。另外分别记录不带分母、不含不可约密度的
