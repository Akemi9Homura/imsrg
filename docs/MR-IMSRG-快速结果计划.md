...
3. 直接积分 `dH/ds=[eta,H]_(0,1,2B)`，记录每步残差和对称性误差。
   按 Vobig 6.5.4 实际定义，正式停止量是不带分母的
   White-NCSM 截断生成元分子，其中不含 `lambda^(2,3,...)`。
   同时必须区分记录含线性 `lambda2` 的严格 `D-D^dagger`，
   用于量化生成元截断差异；它不是这个已发表的近似生成元的
   固定点条件，不得把两种量混称成同一残差。

退出条件：

- `H` 始终 Hermitian，`eta` 始终 anti-Hermitian；
- 目标掩码内的 White-NCSM 截断分子范数相对初值至少下降
  `1e-6`，并同时报告严格 `D-D^dagger` 的变化；
...
| 核 | `Nrefmax` | `||lambda2||` | `s` | `Rgen(s)/Rgen(0)` | `Rstrict(s)/Rstrict(0)` | `E0B(s)` | 后 NCSM 基态 | 原始有限空间基态 | 漂移 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
